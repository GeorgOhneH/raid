    ...
    [(); D + D]:,
{
    ...
    group.bench_function("single read small", |b| {
        b.iter(|| file_handler.read_file("small3"))
    });
    group.bench_function("single read normal", |b| {
        b.iter(|| file_handler.read_file("normal3"))
    });
    group.bench_function("single read large", |b| {
        b.iter(|| file_handler.read_file("large3"))
    });
    file_handler.shutdown();

    let file_handler = prepare_read::<HeadNode<D, C, X>, D, C, X>();
    group.bench_function("distributed read small", |b| {
        b.iter(|| file_handler.read_file("small3"))
    });
    group.bench_function("distributed read normal", |b| {
        b.iter(|| file_handler.read_file("normal3"))
    });
    group.bench_function("distributed read large", |b| {
        b.iter(|| file_handler.read_file("large3"))
    });
    file_handler.shutdown();
    group.finish();
    ...
    [(); D + D]:,
{
    ...
    group.bench_function("single write small", |b| {
        b.iter(|| file_handler.add_file("s".to_string(), &small))
    });
    group.bench_function("single write normal", |b| {
        b.iter(|| file_handler.add_file("n".to_string(), &normal))
    });
    group.bench_function("single write large", |b| {
        b.iter(|| file_handler.add_file("l".to_string(), &large))
    });
    file_handler.shutdown();

    let mut file_handler = prepare_read::<HeadNode<D, C, X>, D, C, X>();
    group.bench_function("distributed write small", |b| {
        b.iter(|| file_handler.add_file("s".to_string(), &small))
    });
    group.bench_function("distributed write normal", |b| {
        b.iter(|| file_handler.add_file("n".to_string(), &normal))
    });
    group.bench_function("distributed write large", |b| {
        b.iter(|| file_handler.add_file("l".to_string(), &large))
    });
    file_handler.shutdown();
    group.finish();
    ...
    [(); D + D]:,
{
    ...
    group.bench_function("single recover", |b| {
        b.iter(|| file_handler.destroy_devices(&failures))
    });
    file_handler.shutdown();
    let file_handler = prepare_read::<HeadNode<D, C, X>, D, C, X>();
